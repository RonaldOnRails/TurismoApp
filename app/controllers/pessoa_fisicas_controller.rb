class PessoaFisicasController < ApplicationController
  ...
  def new
    ...
    @pessoa_fisica.endereco = Endereco.new
  end
  ...
  def create
    ...
    respond_to do |format|
      if @pessoa_fisica.save
        ...
        format.json { render action: 'show', status: :created, location: @pessoa_fisica }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to pessoa_fisicas_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def pessoa_fisica_params
      params.require(:pessoa_fisica).permit(:nome, :sobrenome, :dataNasc, :sexo, :telefone, :email, :cpf, :rg, :facebook, :recebe_email, :status, :nroCompra)
    end
end
